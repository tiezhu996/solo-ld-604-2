...
const {
  ACTIVE_TICKET_STATUS_SQL, OPEN_TICKET_STATUS_SQL, OPEN_REPORT_STATUS_SQL,
} = require('../constants');
...
function overview() {
  const counts = get(`
    SELECT
      ...
      SUM(CASE WHEN status IN (${ACTIVE_TICKET_STATUS_SQL}) THEN 1 ELSE 0 END) AS active,
  ...
  const openReports = get(`SELECT COUNT(*) AS c FROM fault_reports WHERE status IN (${OPEN_REPORT_STATUS_SQL})`);

  const crews = all(`
    SELECT c.id, c.name, c.leader_name, c.duty_status, c.skill_tags,
      (SELECT t.ticket_no FROM repair_tickets t
        WHERE t.crew_id = c.id AND t.status IN (${ACTIVE_TICKET_STATUS_SQL}) LIMIT 1) AS active_ticket_no
    FROM crews c ORDER BY c.id`);

  const typeBreakdown = all(`
    SELECT fault_type AS type, COUNT(*) AS count FROM repair_tickets
    WHERE status IN (${OPEN_TICKET_STATUS_SQL}) GROUP BY fault_type ORDER BY count DESC`);
