...
5. **备件**：申请即校验可用库存（超量 `STOCK_INSUFFICIENT`）；审批出库在事务内二次校验并扣减；
   ...
   可追溯事件；**改派/撤回幂等重试**——相同工单请求同一目标班组或再次撤回时返回当前成功状态
   （`idempotent: true`），不重复回库、不重复写流水、不重复记事件，并发相同请求同样只释放一次；
   无效目标（不存在/未值班/技能不符/在途）保证工单、库存、领用、审计全部保持请求前状态；
   存在未完结领用的工单禁止关闭。每次库存变动写入 `stock_logs` 流水。
...
7. **一致性**：所有多步写入包裹在 SQLite 事务中，提交后原子落盘（临时文件 + rename），
   ...
   工单/报修/领用的状态集合（在途、可吸收、未闭环、未完结领用）在 `backend/src/constants.js`
   单点定义并生成 SQL 片段，全部查询引用同一份定义，不在 SQL 中另写状态字面量
   （有源码守卫测试兜底）。
...
| POST | `/tickets/:id/reassign` | 调度员 | 改派或撤回（同步释放备件占用；同一目标/再次撤回幂等返回） |
...
改派的同事务释放（待审批作废 + 已审批回库）、改派失败全量回滚、释放事件逐条可追溯、
改派/撤回幂等重试（同一目标返回当前状态零副作用）、并发相同请求只释放一次、
复电补派排序与不重复占用、关闭级联、审计留痕、重启后状态一致、状态定义源码守卫。
