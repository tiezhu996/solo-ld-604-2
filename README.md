...
1. **登记合并**：`POST /api/faults` 在同一事务内查找同馈线、同故障类型的未闭环工单——
   存在则报修合并入该工单并按最高等级提升优先级；否则生成新的待派工工单。
...
5. **备件**：申请即校验可用库存（超量 `STOCK_INSUFFICIENT`）；审批出库在事务内二次校验并扣减；
   驳回不占库存；退回回补库存；核销不回补；**改派/撤回自动释放该工单全部未消耗占用**；
...
```bash
cd backend
npm test          # 64 项端到端测试：独立端口 + 临时库，覆盖全闭环与异常分支
```

覆盖：认证与四角色 RBAC、登记合并与等级提升、派工三类校验、状态机越级拦截、
超量领用拦截、审批/驳回/退回/核销的库存联动、改派同步释放、复电补派排序与不重复占用、
关闭级联、审计留痕、重启后状态一致。
